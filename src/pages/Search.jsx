...
export default function Search() {
  ...
  function isHighRisk(food) {
    return (
      food.risks.filter(
        (x) => allergies.includes(x.allergen) && x.risk === "High Risk",
      ).length > 0
    );
  }
  const results =
  ...
  function openDetail(food) {
    ...
    const statusLabel =
      isSafe(food) ? "Safe"
      : isHighRisk(food) ? "Avoid"
      : "Caution";
    const statusColor =
      isSafe(food) ? "#2E7D32"
      : isHighRisk(food) ? "firebrick"
      : "#7B6B00";
    const statusBg =
      isSafe(food) ? "#C8E6C9"
      : isHighRisk(food) === "unsafe" ? "#FF9191"
      : "#FFF9C4";
    addActivity(
      "🔍",
      `Searched: ${food.name}`,
      ...
      <PhoneFrame>
        ...
        <section className="content" style={{ overflow: "auto" }}>
          <div className={`scan-banner ${bannerClass}`}>
            ...
            <h2 style={{ margin: "4px 0 0", color: labelColor }}>
              {safe ?
                "SAFE FOR YOU"
              : hasHighRisk ?
                "UNSAFE"
              ...
              >
                ...
                <span
                  ...
                  style={{
                    ...
                    color:
                      isSafe(item) ? "#2E7D32"
                      : isHighRisk(item) ? "firebrick"
                      : "#7B6B00",
                    background:
                      isSafe(item) ? "#C8E6C9"
                      : isHighRisk(item) ? "#FF9191"
                      : "#FFF9C4",
                  }}
                >
                  {isSafe(item) ?
